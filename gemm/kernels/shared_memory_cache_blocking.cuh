...
__global__ void shared_memory_cache_blocking_gemm_kernel(float* __restrict__ a, float* __restrict__ b, float* __restrict__ c, 
                                                         const int M, const int N, const int K) {


    // Current block is responsible for the calculation of submatrix c[row_start:row_start + BLOCKSIZE, col_start:col_start + BLOCKSIZE].
    ...
    for (int bkIdx = 0; bkIdx < K; bkIdx += BLOCKSIZE) {
        ...
        A_s[OFFSET(thread_row, thread_col, BLOCKSIZE)] = A[OFFSET(thread_row, thread_col, K)];
        B_s[OFFSET(thread_row, thread_col, BLOCKSIZE)] = B[OFFSET(thread_row, thread_col, N)];
        __syncthreads();
        ...
        // Needs to sync again to avoid faster threads updating A_s, B_s before slower threads finish updating sum.
        __syncthreads();
    }
